...
long int randomgen(void)
{	
    int randomseqnum;
	//Current time
	apr_int64_t current_time = apr_time_now();
	randomseqnum = true_random();
	long int max_num=current_time, min_num=randomseqnum ;
	long int random_second_val = 0,low_num=0,hi_num=0;
    if(min_num<max_num)
    {
    low_num=min_num;
    ...
    random_second_val = (rand()%(hi_num-low_num))+low_num;
	return random_second_val;
}
